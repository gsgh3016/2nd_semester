...
def convert_to_midi(midi_out_path: str, seperate_audio_paths: list[str]) -> None:
    """
    ...
    Args:
        ...
        seperate_audio_paths (list[list[str]]): 변환할 오디오 파일들의 파일명 리스트.
    ...
    for file_paths in seperate_audio_paths:
        for file_path in file_paths:
            instrument, _ = os.path.splitext(file_path.split("/")[-1])
            ...
            if not os.path.exists(basic_pitch_path):
                _, midi_data, _ = predict(file_path)
